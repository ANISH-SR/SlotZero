...
const steps = [
  {
    ...
    title: "Connect",
    subtitle: "QuickNode",
    description: "Set up QuickNode Streams to capture real-time SPL token transfer events. Filter to your target tokens and DEXs.",
    code: `const stream = new QuickNodeStream({
  ...
  },
  {
    ...
    title: "Aggregate",
    subtitle: "in ER",
    description: "Events are batched every 10-50ms in MagicBlock Ephemeral Rollups. Calculate fair prices with zero MEV exposure.",
    code: `const batch = await rollup.aggregate({
  window: '50ms',
  tokens: ['JUP', 'ORCA', 'MNDE'],
  anomalyThreshold: 2.0  // Z-score
})
// Batch #1: 10 events, 500K volume`,
  },
  {
    number: "03",
    title: "Detect",
    subtitle: "& alert",
    description: "Monitor live price charts and get instant alerts for volume spikes, whale movements, and pump-and-dumps.",
    code: `dashboard.monitor({
  tokens: ['JUP', 'ORCA', 'MNDE'],
  alerts: ['volume-spike', 'whale-move'],
  latency: '<600ms'
})
// 1,247 events processed today`,
  },
];
        ...
        <div className="relative mb-0 lg:mb-0 grid lg:grid-cols-2 gap-4 lg:gap-12 items-end">
          ...
          <div className="overflow-hidden pb-0 lg:pb-32">
            ...
            <h2 className={`text-6xl md:text-7xl lg:text-[128px] font-display tracking-tight leading-[0.85] transition-all duration-1000 delay-100 ${
              ...
              <span className="block text-white/30">Aggregate.</span>
              <span className="block text-white/10">Detect.</span>
            </h2>
          </div>
